...
def get_windmill_excitation(rpm):
    ...
    rated_T, amplitudes = 2.9e6, generator_torque(rpm) * np.array(
...
def compare_to_data_extracted_from_article():
    ...
    ax1.plot(
        np.linspace(0.1, 25, n_steps), res[0] / 1000, c="b", label="Calculated"
    )
    ax1.scatter(element_1[:, 0], element_1[:, 1], c="r", label="Sopanen & al.")
    ax1.legend()

    ax2.plot(
        np.linspace(0.1, 25, n_steps), res[1] / 1000, c="b", label="Calculated"
    )
