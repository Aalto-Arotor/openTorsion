class Parser:
    ...
    def from_tors(json):
        ...
        def add_part(component):
            ...
            for element in component["elements"]:
                ...
                if element["type"] == "Disk":
                    ...
                    last_disk_key = (
                        list(elements["disks"])[-1]
                        if elements["disks"]
                        else None
                    )
                    if (
                        last_disk_key
                        and elements["disks"][list(elements["disks"])[-1]].node
                        == i
                    ):
                        elements["disks"][last_disk_key].I += element[
                            "inertia"
                        ]
                        elements["disks"][last_disk_key].c += element[
                            "damping"
                        ]
                    else:
                        elements["disks"][elem_name] = Disk(
                            i, I=element["inertia"], c=element["damping"]
                        )
                    ...
                    elements["shafts"][elem_name] = Shaft(
                        i, i + 1, k=element["stiffness"], c=element["damping"]
                    )
                    if "excitation" in element:
                        add_excitation(i, element["excitation"]["values"])
                    ...
                    density = (
                        element["density"] if "density" in element else 8000
                    )
                    elements["shafts"][elem_name] = Shaft(
                        i,
                        i + 1,
                    ...
                    gear_length = (
                        element["teeth"]
                        if "teeth" in element
                        else element["diameter"]
                    )
                    parent_gear = None

                    if (
                        "parent" in element
                        and f'{component["name"]}.{element["parent"]}'
                        in elements["gears"]
                    ):
                        i += 1
                        parent_gear = elements["gears"][
                            f'{component["name"]}.{element["parent"]}'
                        ]
                ...
                if element["name"] in connected_components:
                    connected_comp = find_component_by_name(
                        connected_components[element["name"]], components
                    )
                    add_part(connected_comp)

        # find starting component
        connection_starts = [
            connection[0].split(".")[0] for connection in structure
        ]
        connection_ends = [
            connection[1].split(".")[0] for connection in structure
        ]
        starting_components = list(
            set(connection_starts) - set(connection_ends)
        )
        ...
        starting_component = find_component_by_name(
            starting_components[0], components
        )
