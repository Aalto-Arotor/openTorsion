...
class Plots:
    ...
    def state_space(self, t_in, u1, u2=None):
        ...
        A = np.vstack([np.hstack([A1, A2]), np.hstack([I, Z])])

        M_inv = LA.inv(M)

        B = np.vstack([np.hstack([M_inv]), np.hstack([Z])])
    ...
    def figure_2D(self):
        ...
        for node in nodes_sl:
            ...
            if node in nodes_d:
                ax.add_patch(
                    matplotlib.patches.Polygon(polygon_edges, ec="black", fc="black")
                )
                # disk figure upper part
                ax.add_patch(
                    matplotlib.patches.Polygon(
    ...
    def figure_eigenmodes(self, modes=4):
        ...
        for mode in range(modes):
            ...
            for node in nodes_g:
                axs[mode // 2].plot(
                    s[former_node : node - f],
                    np.real(normalized_mode[former_node : node - f]),
                    color="blue",
                )
                former_node = node - f
                f += 1
            ...
            axs[mode // 2].plot(
                s[former_node:], np.real(normalized_mode[former_node:]), color="blue"
            )
