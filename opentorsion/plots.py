...
class Plots():
    ...
    def plot_campbell(self, frequency_range, modes, excitations=[1, 2, 3, 4]):
        ...
        for i, v in enumerate(modes):
            ...
            plt.text((i*0.1+0.3)*frequency_range, v+0.07*frequency_range,
                     '$f_{i}$'.format(i=freq_num[i]))
        plt.legend(legend)

        for i, v in enumerate(excitations):
            plt.plot([0, frequency_range], [0, v*(frequency_range+50)], color='C0')
            plt.text(0.90*frequency_range, 0.95*(frequency_range+50)*v, '{v}x'.format(v=v))
    ...
    def figure_2D(self):
        ...
        for i, node in enumerate(nodes_sl):
            ...
            if i != len(nodes_sl)-1:
                v = h/2*(shaft_mass[i]-np.min(shaft_mass))/(np.max(shaft_mass-np.min(shaft_mass)))

            plt.text(x0+node*l+m, y0+k+m*0.1+v/2, str(node)) # add node number as text

            if node != nodes_sl[-1]:
                ax.add_patch(matplotlib.patches.Rectangle((x0+node*l, y0+k+v/2), l, -h-v, ec='black')) # shaft figure

            upper_polygon = [(x0+node*l, y0+k+v),
                             (x0+node*l+0.2*l, y0+k+0.05*l+v),
                             (x0+node*l+0.2*l, y0+k+0.1*l+v),
                             (x0+node*l-0.2*l, y0+k+0.1*l+v),
                             (x0+node*l-0.2*l, y0+k+0.05*l+v)] # coordinates for upper disk polygon

            lower_polygon = [(x0+node*l, y0+k-h-v),
                                 (x0+node*l+0.2*l, y0+k-0.05*l-h-v),
                                 (x0+node*l+0.2*l, y0+k-0.1*l-h-v),
                                 (x0+node*l-0.2*l, y0+k-0.1*l-h-v),
                                 (x0+node*l-0.2*l, y0+k-0.05*l-h-v)] # coordinates for lower disk polygon
            if node in nodes_d:
                ax.add_patch(matplotlib.patches.Polygon(upper_polygon, ec='black', fc='black'))
                # disk figure upper part
                ax.add_patch(matplotlib.patches.Polygon(lower_polygon, ec='black', fc='black'))
                # disk figure lower part
        plt.show()
